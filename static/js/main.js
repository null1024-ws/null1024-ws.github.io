...
function applyTheme(theme) {
  const next = theme === "dark" || theme === "light" ? theme : resolveTheme();
  document.body.classList.toggle("dark", next === "dark");
  const hlLink = document.querySelector("link#hl");
  if (hlLink) hlLink.href = `/giallo-${next}.css`;
  return next;
}
...
function enableThemeToggle() {
  ...
  function initGiscusTheme(evt) {
    ...
    toggleGiscusTheme(resolveTheme());
    window.removeEventListener('message', initGiscusTheme);
  }
  window.addEventListener('message', initGiscusTheme);
  if (themeToggle) {
    themeToggle.addEventListener('click', () => toggleTheme(resolveTheme() === "dark" ? "light" : "dark"));
  }
  preferDark.addEventListener("change", e => {
    if (!getStoredTheme()) toggleTheme(e.matches ? "dark" : "light");
  });
  applyTheme(resolveTheme());
  document.addEventListener("prerenderingchange", () => applyTheme(resolveTheme()));
  window.addEventListener("pageshow", () => applyTheme(resolveTheme()));
...
function enablePrerender() {
  ...
  document.querySelectorAll('a.instant').forEach(a => {
    ...
    a.addEventListener('mouseenter', () => {
      timer = setTimeout(() => prefetch(a), 50);
    });
